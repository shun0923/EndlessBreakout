...
void setup() {
	...
	size(800, 1000);
	start();
	startBlockX = (int) (width * 0.05);
	startBlockY = (int) (height * 0.1);
	blockWidth = (int) ((width - startBlockX * 2) / blockNumberW);
	blockHeight = (int) (height * 0.25 / blockNumberH);
	noLoop();
}
...
void start() {
	...
	ballX.add(width * 0.5);
	ballY.clear();
	ballY.add(height * 0.8);
	ballSpeedNow.clear();
	ballSpeedNow.add(10.0);
	ballDegree.clear();
	...
	for(int i = 0; i < blockNumberH; i ++) {
		for(int j = 0; j < blockNumberW; j ++) {
			...
			blockSize[j][i] = 1;
			blockType[j][i] = 0;
			blockBorn[j][i] = 1 + random(100) / 100.0;
		}
	}
}
...
void draw() {
	...
	if(shortCount > 0 && longCount < shortCount) {
		if(countL + count / countTime - shortCount < 1) {
			playerWidth = round(playerWidthStandard * 0.7);
		}else {
			longCount = -1;
			shortCount = -1;
			playerWidth = playerWidthStandard;
	...
	if(slowCount > 0 && fastCount < slowCount) {
		if(countL + count / countTime - slowCount < 1) {
			playerSpeed = round(playerSpeedStandard * 0.5);
		}else {
			fastCount = -1;
			slowCount = -1;
			playerSpeed = playerSpeedStandard;
	...
	if(startText) {
		...
		text("PRESS SPACE BAR!", width * 0.5, height * 0.5);
		startText = false;
	}
...
void calBallCollide() {
	...
	for(int k = 0; k < blockNumberH; k ++) {
		for(int j = 0; j < blockNumberW; j ++) {
			if(existBlock[j][k]) {
				CBCforRect(startBlockX + (j + 0.5) * blockWidth, startBlockY + (k + 0.5) * blockHeight, round(blockWidth * blockSize[j][k]), round(blockHeight * blockSize[j][k]), false, j, k);
			}
		}
	}
}

void CBCforRect(float opponentX, float opponentY, int opponentWidth, int opponentHeight, boolean isPlayer, int j, int i) {
	boolean collideBall = false;
	if((ballX.get(ballNumber) >= (opponentX - opponentWidth * 0.5) && ballX.get(ballNumber) <= (opponentX + opponentWidth * 0.5))
		&& ballY.get(ballNumber) >= (opponentY - ballRad - opponentHeight * 0.5) && ballY.get(ballNumber) <= (opponentY + ballRad + opponentHeight * 0.5)) {
		if(isPlayer) {
			collideUDforP();
		}else {
			collideUD(opponentY);
	...
	if((ballX.get(ballNumber) >= (opponentX - ballRad - opponentWidth * 0.5) && ballX.get(ballNumber) <= (opponentX + ballRad + opponentWidth * 0.5))
		&& ballY.get(ballNumber) >= (opponentY - opponentHeight * 0.5) && ballY.get(ballNumber) <= (opponentY + opponentHeight * 0.5)) {
		collideRL(opponentX);
		if(!isPlayer) {
			existBlock[j][i] = false;
		}
	...
	if(isPlayer) {
		if(dist(ballX.get(ballNumber), ballY.get(ballNumber), opponentX + opponentWidth * 0.5 * signum(ballX.get(ballNumber) - opponentX), opponentY + opponentHeight * 0.5 * signum(ballY.get(ballNumber) - opponentY)) <= ballRad) {
			if(ballX.get(ballNumber) - opponentX - opponentWidth * 0.5 * signum(ballX.get(ballNumber) - opponentX) < ballY.get(ballNumber) - opponentY - opponentHeight * 0.5 * signum(ballY.get(ballNumber) - opponentY)) {
				collideUDforP();
			}else {
				collideRL(opponentX);
			}
	...
	if(!isPlayer && !existBlock[j][i]) {
		...
		blockSize[j][i] = 0;
		score += min(i + 1, blockNumberW - i) * (2 - (float) existBlockNumber / (blockNumberH * blockNumberW)) * (100.0 / countL) * 50;
		if(blockType[j][i] > 0) {
				ballX.add(opponentX);
				ballY.add(opponentY);
				ballSpeedNow.add(10.0);
...
void collideRL(float opponentX) {
...
void collideUD(float opponentY) {
...
void collideUDforP() {
	...
	if((playerX - playerWidth * 0.25) > ballX.get(ballNumber) || (playerX < ballX.get(ballNumber) && (playerX + playerWidth * 0.25) > ballX.get(ballNumber))) {
...
void drawBlock() {
	...
	for(int i = 0; i < blockNumberH; i ++) {
		for(int j = 0; j < blockNumberW; j ++) {
			...
			if(existBlock[j][i]) {
				blockSize[j][i] = min(1, blockSize[j][i] + 0.01);
				rect(startBlockX + (j + 0.5) * blockWidth, startBlockY + (i + 0.5) * blockHeight, round(blockWidth * blockSize[j][i]), round(blockHeight * blockSize[j][i]));
			}else if(countL + count / countTime - deleteBlockTime[j][i] >= blockBorn[j][i]) {
				blockBorn[j][i] += random(600) / 100.0 + 1;
				deleteBlockTime[j][i] = -1;
				existBlock[j][i] = true;
...
void drawPlayer() {
	...
	playerX = constrain(playerX, 0 - playerWidth * 0.4, width + playerWidth * 0.4);
	rectMode(CENTER);
	fill(255, 0, 100);
...
void drawScore() {
	...
	textSize(30);
	textAlign(LEFT, TOP);
	text("SCORE : "+score+"\nTIME : "+(round((millis() / 1000.0 - startCount) * 10.0) / 10.0), 10, 10);
}
